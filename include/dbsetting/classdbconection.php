...
class dblms {
	...
	public function getTableIdName($table){ 
		$colName        = $this->querylms("SELECT GROUP_CONCAT(COLUMN_NAME) col_name
											FROM INFORMATION_SCHEMA.COLUMNS
											WHERE TABLE_NAME = '".$table."'");
		$row = mysqli_fetch_array($colName);
		$colArray = explode(',',$row['col_name']);
		$colNameArray = array();
		$orderId = '';
		foreach($colArray as $key => $val):
			if (strpos($val,'_id')) {
				array_push($colNameArray,$val);
				$orderId = $val;
			}
			if (strpos($val,'_name')) {
				array_push($colNameArray,$val);
		...
		$data     = $this->getRows($table.' t', $filters);
        return !empty($data)?$data:false; 
    }
