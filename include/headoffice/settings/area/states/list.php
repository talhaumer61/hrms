...
$condition = array ( 
                          'select' 	=> 's.state_id,s.state_status,s.state_name,s.state_codedigit,s.state_codealpha,s.state_latitude,s.state_longitude,s.id_country, c.country_name'
                        , 'join' 	=> "INNER JOIN ".COUNTRIES." c ON c.country_id = s.id_country"
                        ,'where' 	=> array( 
                                            's.is_deleted' 	=> 0 
                                        ) 
                        ,'search_by'    =>  'AND (s.state_name LIKE "%'.$search_word.'%")'
                        ,'order_by' 	=> 's.state_id ASC'
                        ,'return_type' 	=> 'count' 
                    ); 
...
<div class="card">
    <div class="card-header">
        <div class="d-flex align-items-center">
            <h5 class="card-title mb-0 flex-grow-1"><i class="ri-file-paper-2-fill align-bottom me-1"></i>State List</h5>
            <div class="flex-shrink-0">
                <a class="btn btn-primary btn-sm" onclick="showAjaxModalZoom(\'include/modals/states/add.php?ordering='.get_ordering(STATES).'\');"><i class="ri-add-circle-line align-bottom me-1"></i>State</a>
            </div>
        </div>
    </div>
    <div class="card-body">        
        <div class="row justify-content-end">
            <div class="col-3">
                <form action="states.php" class="form-horizontal" id="form" enctype="multipart/form-data" method="get" autocomplete="off" accept-charset="utf-8">
                    <div class="input-group mb-3">
                        <input type="text" class="form-control" placeholder="State Name" name="search_word" value="'.$search_word.'">
        ...
        $condition['order_by'] = "s.state_id ASC LIMIT " . ($page - 1) * $Limit . ",$Limit";
        $condition['return_type'] = 'all';

        $states     = $dblms->getRows(STATES.' s', $condition);
        if ($states) {
            echo'
