...
if (!empty($_GET['search_word'])) {
    ...
    $search_query   .= " AND (cat_name LIKE '%".$search_word."%' OR cat_status LIKE '%".$search_word."%')";
    $filters        .= '&search_word='.$search_word.'';
}

$condition = array(
                     'select'       =>  'cat_id,cat_status,cat_name'
                    ,'where'        =>  array(
                                                'is_deleted'  => 0
                                            )
...
$count     = $dblms->getRows(QUESCATEGORY, $condition);
echo'
<div class="card">
    ...
    <div class="card-body">        
        ...
        $condition['order_by'] = "cat_id DESC LIMIT " . ($page - 1) * $Limit . ",$Limit";
        $condition['return_type'] = 'all';
        $regions     = $dblms->getRows(QUESCATEGORY, $condition);
        if ($regions) {
            echo'
                                ...
                                <td class="text-center">
                                    <div class="dropdown">
                                        ...
                                        <ul class="dropdown-menu dropdown-menu-end" style="cursor: pointer;">
                                            <li><a class="dropdown-item" href="'.moduleName().'.php?id='.$row['cat_id'].'"><i class="ri-pencil-fill align-bottom me-2 text-muted"></i> Edit</a></li>
                                            <li><a class="dropdown-item" onclick="confirm_modal(\'questions_category.php?deleteid='.$row['cat_id'].'\');"><i class="ri-delete-bin-fill align-bottom me-2 text-muted"></i> Delete</a></li>
                                        </ul>
                                    </div>
                                </td>
                            </tr>';
